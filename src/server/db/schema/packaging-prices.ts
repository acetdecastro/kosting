...
export const packagingPrices = pgTable(
  "packaging_prices",
  {
  ...
  (t) => [
    index("idx_packaging_prices_effective_from").using(
      "btree",
      t.effectiveFrom.asc().nullsLast().op("date_ops"),
    ),
    index("idx_packaging_prices_item_id").using(
      "btree",
      t.packagingItemId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_packaging_prices_supplier_id").using(
      "btree",
      t.supplierId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_packaging_prices_user_id").using(
      "btree",
    ...
    foreignKey({
      ...
      name: "packaging_prices_package_unit_id_fkey",
    }),
    foreignKey({
      columns: [t.packagingItemId],
      foreignColumns: [packagingItems.id],
      name: "packaging_prices_packaging_item_id_fkey",
    }),
    foreignKey({
      columns: [t.supplierId],
      foreignColumns: [suppliers.id],
      name: "packaging_prices_supplier_id_fkey",
    }),
  ],
);
